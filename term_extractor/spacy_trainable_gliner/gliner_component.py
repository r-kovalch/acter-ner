...
def build_gliner_model(model_name: str, labels: list[str]) -> Model:
    ...
    """
    # Pass 'labels' directly—no need for a separate set_labels() call
...
# 2) Define the TrainablePipe subclass (unchanged)
class GLiNERPipe(TrainablePipe):
    def __init__(self, nlp, name, model_name, labels, threshold: float = 0.5):
...
# 3) Factory registration (unchanged)
@Language.factory(
    "gliner",
    default_config={
...
def make_gliner(nlp: Language, name: str, model_name: str, labels, threshold: float):
